...
data.only.maingwas <- data.only.35 %>% 
  filter(`Trait identifier` %ni% gwas.main)
...
### Add meta data
df.ldsc_cts <- data.only.maingwas %>% left_join(df.metadata, by="annotation") # add meta data
df.ldsc_cts
...
p <- multi_ES_and_GWAS_scatter_plot(df.plot.tax_order)
p
### Save
file.out <- sprintf("figs/sup-fig-detect-relevance-scatter-other-gwas.tabula-muris.199827.png")
ggsave(p, filename=file.out, width=16, height=10)
